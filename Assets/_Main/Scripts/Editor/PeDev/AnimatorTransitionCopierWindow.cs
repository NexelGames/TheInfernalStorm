...
namespace Game.EditorTools {
	class AnimatorTransitionCopierWindow : EditorWindow {
		[MenuItem("Tools/Windows/Animator Transition Copier")]
		...
		readonly string AnimStateCondition = "AnimState";
		...
		void OnAnimatorTransitionCopierGUI() {
			...
			if (!IsCurrentLayerValid) {
				EditorGUILayout.HelpBox($"Please assign AnimatorController to perform copy/paste operations.", MessageType.Warning);
			}
			...
			if (selectedTransitions.Length > 0) {
				var conditions = selectedTransitions[0].conditions;
				if (conditions.Length > 0) {
					for (int i = 0; i < conditions.Length; i++) {
						if (conditions[i].parameter == AnimStateCondition) {
							EditorGUILayout.LabelField("Animation State:");
							var stateValue = EditorGUILayout.EnumPopup((AnimationStates)(int)conditions[i].threshold);
							conditions[i].threshold = (int)((AnimationStates)stateValue);
							conditions[i].mode = AnimatorConditionMode.Equals;
							break;
						}
					}
					selectedTransitions[0].conditions = conditions;
				}
			}
